module.exports = {
    ...
    scriptModifyUserInfo: "UPDATE `users` SET `userPhoneNumber` = CASE WHEN ? <> 0 THEN ? ELSE `userPhoneNumber` END,`userNationality` = CASE WHEN ? <> '' THEN ? ELSE `userNationality` END,`userPrefixNational` = CASE WHEN ? <> '' THEN ? ELSE `userPrefixNational` END,`userName` = CASE WHEN ? <> '' THEN ? ELSE `userName` END WHERE `userId` = ?;",
    scriptInsertResume: "INSERT INTO `resumes` (`userId`, `resumeDescription`, `resumeTimeExperience`, `resumeTitleLabor`) VALUES (?, ?, ?, ?);",
    scriptUpdateResume: "UPDATE `resumes` SET `resumeDescription` = ?, `resumeTimeExperience` = ?, `resumeTitleLabor` = ? WHERE `userId` = ? AND `resumeId` = ?;",
    scriptInsertLaborResumeRelationship: "INSERT INTO `laborsresumes` (`resumeId`, `laborId`) VALUES (?, ?);",
    scriptDeleteLaborResumeRelationship: "DELETE FROM `laborsresumes` WHERE `resumeId` = ? AND `laborId` IN (?);",
    scriptSelectLaborResumeRelationship: `SELECT laborId FROM laborsresumes WHERE resumeId = ?;`,
 ...
 ORDER BY listingValue DESC;`,
    ...
    scriptGetEmployerPendingJobs: "SELECT f.jobOfferId,f.jobOfferDescription,f.jobOfferDateAtCreate,f.jobOfferDateAtWork,f.jobOfferEstimatePrice,f.jobOfferTittle, j.jobId, j.jobPrice, j.jobStatus, j.jobOrderId FROM joboffers f left join jobs j on j.jobOfferId=f.jobOfferId WHERE (j.jobStatus = 0 or j.jobStatus = 1) and f.userId=?;",
    scriptGetJobsPendingOfWorker: "SELECT jo.*, j.jobId, j.jobDate, j.jobStatus, j.jobPrice, j.jobOrderId FROM joboffers jo RIGHT JOIN ( SELECT jobId, jobOfferId, jobDate, jobStatus, jobPrice, jobOrderId, userId FROM jobs WHERE userId = ? and (jobStatus = 0 or jobStatus = 1) GROUP BY jobId, jobOfferId, jobDate, jobStatus, jobPrice, jobOrderId, userId ) AS j ON jo.jobOfferId = j.jobOfferId;",
    scriptGetEmployerDoneJobs: "SELECT f.jobOfferId,f.jobOfferDescription,f.jobOfferDateAtCreate,f.jobOfferDateAtWork,f.jobOfferEstimatePrice,f.jobOfferTittle, j.jobId, j.jobPrice, j.jobStatus, j.jobOrderId FROM joboffers f left join jobs j on j.jobOfferId=f.jobOfferId WHERE j.jobStatus = 4 and f.userId=?;",
    scriptGetJobsDoneByWorker: "SELECT jo.*, j.jobId, j.jobDate, j.jobStatus, j.jobPrice, j.jobOrderId FROM joboffers jo RIGHT JOIN ( SELECT jobId, jobOfferId, jobDate, jobStatus, jobPrice, jobOrderId, userId FROM jobs WHERE userId = ? and jobStatus=4 GROUP BY jobId, jobOfferId, jobDate, jobStatus, jobPrice, jobOrderId, userId ) AS j ON jo.jobOfferId = j.jobOfferId;",
